...
API_KEY = "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"
...
def get_closest_match(query, choices):
    match = difflib.get_close_matches(query, choices, n=1, cutoff=0.85)
...
def check_personal_data(query):
    ...
    for key, phrases in personal_questions.items():
        if key in ["user_name_query", "user_name_set"]:
            continue
        matched_phrase = get_closest_match(query, phrases)
        if matched_phrase:
            if key == "time":
                return get_current_time()
            return PERSONAL_DATA[key]
...
def is_study_related(query):
    ...
    study_keywords = {
        ...
        'synthesize', 'apply', 'interpret', 'illustrate', 'demonstrate', 'justify',
    }

    query_words = set(query.lower().split())
    return len(query_words & study_keywords) > 0
...
def query_deepseek(prompt):
    ...
    json_data = {"model": "deepseek/deepseek-r1-distill-llama-70b:free", "messages": messages, "temperature": 0.7}
...
def text_mode():
    print("Text chat mode activated. Type 'exit' to switch modes or 'clear history' to clear chat history.")
    while True:
        user_input = input("You: ")
        if user_input.lower() == "exit":
            break
        response = query_deepseek(user_input)
        print(f"Kiko: {response}")
        speak(response)
...
def login():
    ...
    flash(f"Welcome {name} ({user_type})!", "success")
...
@app.route('/logout')
def logout():
    login_iso = session.get('login_time')
    student_id = session.get('user_id')

    if login_iso and student_id:
        try:
            login_dt = datetime.fromisoformat(login_iso)
            logout_dt = datetime.now()
            duration = (logout_dt - login_dt).total_seconds() / 60

            conn = get_db_connection()
            conn.execute('INSERT INTO session_activity (student_id, login_time, logout_time, duration_minutes) VALUES (?, ?, ?, ?)',
                        (student_id, login_dt.isoformat(), logout_dt.isoformat(), duration))
            conn.commit()
            conn.close()
            print(f"Logged out: Duration {duration} minutes for {student_id}")  # Debug print
        except Exception as err:
            print("Session-activity insert failed:", err)
...
@app.route("/chat_api/<chat_id>/typing", methods=["POST"])
def typing_status(chat_id):
    data = request.get_json()
    conn = get_db_connection()
    conn.execute("""
        DELETE FROM chat_messages 
        WHERE chat_id=? AND sender_id=? AND is_typing=1
    """, (chat_id, session['user_id']))
    if data['is_typing']:
        conn.execute("""
            INSERT INTO chat_messages 
            (chat_id, sender_role, sender_id, is_typing)
            VALUES (?,?,?,?)
        """, (chat_id, data['sender_role'], session['user_id'], 1))
    conn.commit()
    conn.close()
...
QUIZ_API_KEY = "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"
